...
export default class Crop extends Plugin {
  ...
  getRectWidth = () => {
    return this.rect
      ? this.rect.getClientRect({ skipTransform: false }).width
      : 0;
  };
  ...
  getRectHeight = () => {
    return this.rect
      ? this.rect.getClientRect({ skipTransform: false }).height
      : 0;
  };
  ...
  adjustToolbarPosition = (stage: any) => {
    ...
    if (left < 0) left = 0;
    if (left > stage.width() - toolbarWidth)
      left = stage.width() - toolbarWidth;
    if (top < 0) top = 0;
  ...
  createCropToolbar = (
    ...
    const cropToolbarStyle =
      'position: absolute; z-index: 1000; box-shadow: 0 2px 8px 0 rgba(0, 0, 0, 0.15);' +
    ...
    const btnStyle =
      'display: inline-block; width: 32px; height: 24px; border: 1px solid #C9C9D0;' +
    ...
    $cancelBtn.setAttribute(
      'style',
      btnStyle + 'background: #FFF; margin: 0 8px 0 10px;',
    );
    ...
    $sureBtn.setAttribute(
      'style',
      btnStyle + 'background: #007AFF; color: #FFF;',
    );
