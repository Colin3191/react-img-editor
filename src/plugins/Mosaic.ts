...
export default class Mosaic extends Plugin {
  ...
  getTilesByPoint = (x: number, y: number, strokeWidth: number) => {
    ...
    let startRow = Math.max(
      0,
      Math.floor(y / tileHeight) - Math.floor(strokeWidth / 2),
    );
    const startColumn = Math.max(
      0,
      Math.floor(x / tileWidth) - Math.floor(strokeWidth / 2),
    );
  ...
  onDrawStart = (drawEventParams: DrawEventParams) => {
    ...
    for (let i = 0; i < this.tileRowSize; i++) {
      for (let j = 0; j < this.tileColumnSize; j++) {
        ...
        // 转换为像素图形下，起始像素位置
        const pixelPosition =
          (this.width * tileHeight * tile.row + tile.column * tileWidth) * 4;
        // 转换为像素图形下，包含多少行
        ...
        for (let i = 0; i < pixelRowAmount; i++) {
          ...
          data = [
            ...data,
            ...imageData.data.slice(position, position + tile.pixelWidth * 4),
          ];
        }
        tile.data = data;
