...
import type PubSub from '../common/PubSub';
...
export default class Text extends Plugin {
  ...
  removeTextareaBlurModal = () => {
    ...
    if (textareaBlurModal) {
      textareaBlurModal.removeEventListener(
        'click',
  ...
  disableTransform = (
    drawEventParams: DrawEventParams,
    node: any,
    remove?: boolean,
  ) => {
  ...
  onClick = (drawEventParams: DrawEventParams) => {
    ...
    const fontSize =
      paramValue && paramValue.fontSize
        ? paramValue.fontSize
        : this.defaultParamValue.fontSize;
    const color =
